...
#Task: make your bot reply to the most highly upvoted comment in a thread that it hasn't already replied to.
#Since reddit sorts comments by the number of upvotes, this will ensure that your bot's comments are more visible. 
#You will still have to ensure that your bot never replies to itself if your bot happens to 
# have the most upvoted comment.

    try: 
# comment = random.choice(comments_without_replies)
        comment = sorted(comments_without_replies, key=lambda x: x.score, reverse=True)[0]  
        text= generate_comment()
        comment.reply(text)
    except praw.exceptions.APIException:
            print('will not reply to deleted comment')
    except IndexError:
            print('replied to all comments not mine')